...
| `data/` | Packaged `.rda` artifacts read by the transforms. | Regenerated by pipelines (XZ compression). |
...
rapidly on plotting layers or statistical summaries.
...
- **Entrypoint:** `run_binned_pipeline()` orchestrates `frsrr::frsr_bin()`
  ...
  out-of-bin (OOB) error, and saves:
...
- **Known gap:** `compute_oob_performance()` and `analyze_bin_changes()` live
  only inside the pipeline/test expectations. Promote these helpers into `R/`
  (probably alongside `find_optimal_buckets`) so tests can import them without
  sourcing the pipeline script manually.
...
- **Broken expectations:** The test file references
  `visualfrsr:::analyze_bin_changes` and `visualfrsr:::compute_oob_performance`
  yet those functions are only defined inside `data-raw/pipelines/binned.R`.
  Promote them into `R/binned.R` (or a new helper file), then re-export/int
  integrate tests.
...
1. **Function placement drift:** Several computational helpers still live
   inside `data-raw/pipelines/*` while tests or other modules expect them
   inside the package namespace. Consolidate helpers under `R/` and keep
   pipelines as thin orchestrators.
2. **Data availability:** The repo currently commits only the CSV-to-RDA
   conversions for bin-related datasets (`equal*.rda`, etc.). If agents need
   `deconstructed`, `bin_samples`, etc., re-run the corresponding pipeline and
   commit the outputs (or document how to regenerate them on demand).
3. **Diagnostics folder:** `R/ToTransform/diagnostics.R` is untracked but
   present locally. Treat it as experimental—do not assume availability in CI.
4. **Pkgdown:** `_pkgdown.yml` covers only a subset of exported functions.
   Update it when new transforms/plots become public to keep site docs synced.
5. **Threading defaults:** `.Renviron.project` caps OpenMP/MKL threads at 1.
   If agents change sampler workloads, revisit these values to prevent
   artificial bottlenecks.
6. **frsrr dependency:** Building `{frsrr}` requires clang++ with C++20 and
